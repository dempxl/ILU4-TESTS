...
\author{\href{mailto:loganlarrouxpro@gmail.com}{Logan Larroux}, \href{mailto:loay.younes@utoulouse.fr}{Loay Younes}, \href{mailto:manolo.sardo@example.com}{Manolo Sardó}\\\href{https://univ-tlse3.fr}{Université de Toulouse}}
\date{\today}
...
\begin{description}
    ...
    \item[Main~:] point d'entrée orchestrant l'ensemble. Il construit la liste de tous les cas à tester --~les deux implémentations de référence, puis les 35 mutants simples, puis les $35 \times 35 = 1225$ mutants doubles~-- et exécute \mintinline{text}`RunTest` sur chacun. Seuls les mutants ayant passé tous les tests (mode \mintinline{text}`lazy`) sont affichés dans le rapport final.
\end{description}
...
Dans un premier temps, les mutants sont créés, compilés et stockés dans une liste. Puis, tous les tests sur tous les mutants sont exécutés. Cette approche en deux étapes est inefficace, car elle génère et compile les mutants et les stocke sans savoir s'ils survivront ou non. Le \textit{"faible"} nombre de mutants (1260) rend cependant ce choix moins catastrophique que s'il avait été question de dizaines de milliers de mutants. Une optimisation possible serait de générer et tester les mutants un par un, en mode \mintinline{text}`lazy`, pour éviter de stocker des mutants manifestement défectueux.
...
Les mutants ont été produits manuellement en appliquant des \textbf{opérateurs de mutation relation\-nels} à la version corrigée \mintinline{text}`TrianglesClassifierCorrected`. Les opérateurs utilisés remplacent chaque opérateur relationnel (\mintinline{text}`<=`, \mintinline{text}`<`, \mintinline{text}`==`, \mintinline{text}`!=`, \mintinline{text}`>`, \mintinline{text}`>=`, \mintinline{text}`&&`, \mintinline{text}`||`) par un autre opérateur de la même famille, sur chaque condition de la fonction.

35 mutations élémentaires ont été définies (table \mintinline{text}`MUTATIONS` dans \mintinline{text}`Main`). Elles ciblent~:
...
\begin{itemize}[label=$\bullet$]
    ...
    \item les opérateurs logiques (\mintinline{text}`&&`, \mintinline{text}`||`) dans les conditions composées.
\end{itemize}

En plus des 35 mutants d'ordre 1, tous les couples de mutations ont été générés, soit $35 \times 35 = 1225$ mutants d'ordre 2, pour un total de \textbf{1260 mutants}.
...
Parmi les 1260 mutants générés, \textbf{aucun ne survit}. Tous sont tués par au moins un cas de test. Les tests aux limites (VL1, VL5, VL9, VL22) détectent les mutations sur la condition de garde.
...
Beaucoup de mutants survivants atteignent une \textbf{couverture structurelle de 100\%} (57 mutants sans compter \mintinline{text}`TrianglesClassifierCorrected`) (zéro branche manquée, zéro ligne manquée), identique à celle de \mintinline{text}`TrianglesClassifierCorrected`. Ce résultat illustre une limite fondamentale de la couverture structurelle~: une couverture à 100\% des branches ne garantit pas l'absence de défauts sémantiques.
